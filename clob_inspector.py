...
def _sample_texts(scur, qtab, clob_col, limit, max_chars=6000):
    """Fetch up to `limit` CLOB values as Python strings.
    Uses a dedicated cursor with an output type handler that fetches CLOBs
    directly as strings (DB_TYPE_LONG) — no LOB locators, no 4000-byte SQL
    cast ceiling, and immune to locator-lifetime quirks during iteration."""
    import oracledb
    ...
    c = f'"{clob_col}"'
    cur = scur.connection.cursor()
    try:
        cur.outputtypehandler = _clob_as_str
        cur.execute(f"SELECT {c} FROM {qtab} "
                    f"WHERE {c} IS NOT NULL AND ROWNUM <= {limit}")
        return [(txt or "")[:max_chars] for (txt,) in cur]
    finally:
        cur.close()
...
def inspect(data_source, table, clob=None, sample_rows=0, run_id=None):
    schema_opt, _, tname = table.strip().upper().rpartition(".")
    table = tname                                  # store unqualified, always
    ...
    try:
        ...
        scur = sconn.cursor()
        # resolve the owning schema: explicit schema wins; otherwise try
        # each stage's schema until the table exists there
        candidates = ([f"{schema_opt}.{table}"] if schema_opt else
                      [_qual(data_source, st, table)
                       for st in ("DWH", "STG2", "STG1", "SRC")])
        qtab = None
        for cand in candidates:
            if _col_types(scur, cand):
                qtab = _phys(cand)
                break
        if not qtab:
            raise RuntimeError(
                f"table {table} not found in any stage schema "
                f"(tried: {', '.join(candidates)})")
        log.info("resolved %s -> %s", table, qtab)
